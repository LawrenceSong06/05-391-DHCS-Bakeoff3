...
function saveLog() {
  writeFileSync(LOG_PATH, JSON.stringify({ best, samples }, null, 2), "utf8");
  if (best) {
    ...
    writeFileSync(
      "./best_params.js",
      `// Best loss: ${best.loss} (after ${samples.length} runs)\nexport const params = {\n${entries}\n};\n`,
      "utf8",
    );
  }
}
...
function weightedMeanLogSpace(keys) {
  ...
  return Object.fromEntries(
    keys.map((k) => {
      const logMean =
        samples.reduce(
          (acc, s, i) => acc + Math.log(s.params[k]) * weights[i],
          0,
        ) / totalW;
...
function isDuplicate(candidate) {
  return samples.some((s) =>
    Object.keys(candidate).every(
      (k) => Math.abs(Math.log(s.params[k]) - Math.log(candidate[k])) < 1e-9,
    ),
  );
}
 ...
 */
export function init(liveParams) {
  ...
  if (isDuplicate(currentParams)) {
    console.log(
      "[optimizer] Current params already tested — generating new candidate.",
    );
  ...
  if (best) {
    ...
    console.log(
      `[optimizer] ↕  delta from best: ${fmtDelta(currentParams, best.params)}`,
    );
  }
