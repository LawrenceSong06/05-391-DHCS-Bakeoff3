type movieData = { // the type of a single movie in the movies list
	title: string,
	movieTimes: Array<string>,
	movieLength: number,
	genres: Array<string>,
	description: string,
	actors: Array<string>
...
type userData = { // the type of a movie selection (what the user chooses/submits)
	movie: movieData, 
	movieTime: string, 
...
type trialData = { // the type of an entire trial result for judging (i.e., with teamName and elapsedTime)
	userData: userData,
	teamName: string,
	elapsedTime: number,
	movieSet: {seed: number, quantity: number},
...
class Trial {
	...
	constructor(teamName: string, testMovies?: movieData[], verbose?: boolean) {
	...
	validateData(userData) : boolean {
		...
		if (dataIsValid == true) return true;
		else {
			console.log(`Submitted data should be of the form: 
{
	...
	submitMovieChoice(userData: userData) {
		if (!this.submitted) {
	...
	postToServer(data : trialData) : void {
		if (this.verbose) console.log("Sending to server.");
		...
		.then(json => {
			data.result = json;
			if (this.verbose) console.log("Received from server.");
	...
	async getMovies() : Promise<movieData[]> {
		...
		.then(json => { // once it's parsed...
			if (this.verbose) console.log("Received from server:", json);
