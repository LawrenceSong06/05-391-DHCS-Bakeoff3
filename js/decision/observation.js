...
exports.registry = exports.Registry = exports.TicketsObserver = exports.ActorObserver = exports.GenreObserver = exports.TagObserver = exports.ShowtimeObserver = exports.Observer = void 0;
...
class ShowtimeObserver extends Observer {
    /**
     *
     */
    constructor() {
    ...
    coalescePassedIntervals() {
        if (this.passedIntervals.length === 0)
            return;
    ...
    getProbability(movie) {
        ...
        risk *= Math.exp(-unaccountedMovieTime * params_js_1.params.showtimeTruePoisson);
        for (let interval of this.failedIntervals) {
            let intervalLength = interval[1] - interval[0];
            let blockerMustBeIn = intervalLength - this._computeOverlap(movie, interval[0], interval[1]);
            let blockerPercentageChance = blockerMustBeIn / intervalLength; // < 1, > 0
            risk *= Math.pow(blockerPercentageChance, params_js_1.params.showtimeFailedExpectedBlockers);
        }
        return Math.pow(risk, params_js_1.params.showtimePower);
    }
}
exports.ShowtimeObserver = ShowtimeObserver;
...
class TagObserver extends Observer {
    constructor(tags) {
         ...
         */
        this.perceptronWeights = new Map();
        this.bias = 0;
        this.prevTagLists = [];
        this.prevResults = [];
        for (let tag of tags) {
...
class GenreObserver extends TagObserver {
    constructor(genres) {
        super(genres);
    }
    /**
     ...
     */
    getProbability(movie) {
        if (this._isRuledOut(movie.genres))
            return 0;
        let weight = this.bias;
        for (let genre of movie.genres) {
            weight += this.perceptronWeights.get(genre);
        }
        return Math.pow(1 / (1 + Math.exp(-weight * params_js_1.params.genreSigmoidSharpness)), params_js_1.params.genrePower);
    }
    /**
     *
     * @param {Movie} movie
...
exports.GenreObserver = GenreObserver;
class ActorObserver extends TagObserver {
    constructor(actors) {
        super(actors);
    }
    /**
    ...
    getProbability(movie) {
        let weight = this.bias;
        if (this._isRuledOut(movie.actors))
            return 0;
        for (let actor of movie.actors) {
            weight += this.perceptronWeights.get(actor);
        }
        return Math.pow(1 / (1 + Math.exp(-weight * params_js_1.params.actorSigmoidSharpness)), params_js_1.params.actorPower);
    }
    /**
     *
     * @param {Movie} movie
...
exports.ActorObserver = ActorObserver;
class TicketsObserver extends Observer {
    constructor() {
        super();
        //we have observed that you need AT LEAST THIS MANY TICKETS
        this.maxConfFail = 0;
        this.minConfPass = 999;
    }
    observe(observation, passed) {
    ...
    getProbability(movie) {
        ...
        let lerp = (movie.tickets - this.maxConfFail) /
            (this.minConfPass - this.maxConfFail);
        return Math.pow(lerp, params_js_1.params.ticketsPower);
    }
}
exports.TicketsObserver = TicketsObserver;
class Registry {
    constructor() {
        this.moves = [];
        this.map = {};
    ...
    computeSeekingEnergyWeight(moveHistory, probs) {
        ...
        let energy = 0;
        for (let i = 0; i < movie_js_1.movies.length; i++) {
            if (seenOnce.has(movie_js_1.movies[i])) {
                energy += params_js_1.params.s1_energy * probs[i];
            }
            if (seenTwice.has(movie_js_1.movies[i])) {
                energy += params_js_1.params.s2_energy * probs[i];
            }
            if (seenThrice.has(movie_js_1.movies[i])) {
                energy += params_js_1.params.s3_energy * probs[i];
            }
            if (completed.has(movie_js_1.movies[i])) {
                energy += params_js_1.params.s4_energy * probs[i];
            }
        }
        return energy;
    }
...
exports.registry.register(ActorObserver, movie_js_1.movies);
exports.registry.register(GenreObserver, movie_js_1.movies);
exports.registry.register(ShowtimeObserver, movie_js_1.movies);
exports.registry.register(TicketsObserver, movie_js_1.movies);
